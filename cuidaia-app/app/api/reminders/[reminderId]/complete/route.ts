...
export async function POST(request: NextRequest, { params }: { params: { reminderId: string } }) {
  try {
    ...
    console.log("[v0] Completing reminder:", reminderId)
    ...
    for (const userId in reminders) {
      ...
      if (reminderIndex !== -1) {
        userReminders[reminderIndex].lastCompleted = new Date().toISOString()
        reminderFound = true
        break
      }
    }
    ...
    console.log("[v0] Reminder completed successfully:", reminderId)

    return NextResponse.json({
      message: "Recordatorio completado exitosamente",
    })
  } catch (error) {
